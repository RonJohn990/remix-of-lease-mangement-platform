...
import { differenceInDays, addMonths, addQuarters, addYears, format, parseISO, isBefore, isAfter, endOfMonth, subDays } from 'date-fns';
...
/**
 * For arrears, get the last date of the period ending at this payment date.
 * Monthly: end of the payment month
 * Quarterly: end of month of the payment date (which is start of next quarter minus 1 day)
 * Half-Yearly / Annual: similarly end of the month before next period starts
 */
function getEndOfPeriodDate(paymentDate: Date, frequency: string): Date {
  // paymentDate is the start-of-period date generated by getPaymentDates.
  // The end of this period is the day before the next period starts,
  // but for display we use the last day of the month of the period end.
  switch (frequency) {
    case 'Monthly': return endOfMonth(paymentDate);
    case 'Quarterly': return subDays(addQuarters(paymentDate, 1), 1);
    case 'Half-Yearly': return subDays(addMonths(paymentDate, 6), 1);
    case 'Annual': return subDays(addYears(paymentDate, 1), 1);
    default: return endOfMonth(paymentDate);
  }
}
...
export function computeLease(lease: Lease): LeaseComputation {
  ...
  if (modifications.length === 0) {
    const result = buildScheduleSegment(
      ...
      initialLiability, initialROU, lease.security_deposit || 0, 1, paymentTiming, lease.payment_frequency
    );
    return {
  ...
  for (let mi = 0; mi < modifications.length; mi++) {
    ...
    if (preModPayments.length > 0) {
      const preResult = buildScheduleSegment(
        ...
        currentLiab, currentROU, lease.security_deposit || 0, periodCounter, paymentTiming, lease.payment_frequency
      );
